...
% start time
x1s = find(abs(acc_time-0)<0.002);  x1s = x1s(1);
x1e = find(abs(acc_time-10)<0.001); x1e = x1e(1);

xS = find(abs(acc_time-3.6)<0.002); xS = xS(1);
xE = find(abs(acc_time-7.5)<0.001); xE = xE(1);

y1s = find(abs(acc_time-45)<0.002);  y1s = y1s(1);
y1e = find(abs(acc_time-55)<0.001);  y1e = y1e(1);

yS = find(abs(acc_time-46.15)<0.002); yS = yS(1);
yE = find(abs(acc_time-48.5)<0.002); yE = yE(1);

z1s = find(abs(acc_time-90)<0.002);  z1s = z1s(1);
z1e = find(abs(acc_time-100)<0.001); z1e = z1e(1);
...
figure
subplot(311), plot(acc_time, acc_data_x);
subplot(312), plot(acc_time, acc_data_y);
subplot(313), plot(acc_time, acc_data_z);


accx = acc_data_x(x1s:x1e);% -0.1364;%- 0.1427; %mean(acc_data_x(xS:xE));
accy = acc_data_y(z1s:z1e);% +0.0023;%+ 0.0043; %- mean(acc_data_y(xS:xE));
accz = acc_data_z(y1s:y1e);% +0.6162;%+ 0.6371; %- mean(acc_data_z(xS:xE));

accxt = acc_time(x1s:x1e);% -0.1364;%- 0.1427; %mean(acc_data_x(xS:xE));
accyt = acc_time(z1s:z1e);% +0.0023;%+ 0.0043; %- mean(acc_data_y(xS:xE));
acczt = acc_time(y1s:y1e);% +0.6162;%+ 0.6371; %- mean(acc_data_z(xS:xE));


accTx = acc_time(xS:xE);
accTy = acc_time(zS:zE);
accTz = acc_time(yS:yE);
% ========== Start End Time Acceleration ========== 

timex = unique(sort(accTx),'rows'); timex = timex - timex(1);
timey = unique(sort(accTy),'rows'); timey = timey - timey(1);
timez = unique(sort(accTz),'rows'); timez = timez - timez(1);

%timeDiff = diff(accT);

%
[PP1, VV1, AA1] = groundtruth1Dx(timex);
[PP2, VV2, AA2] = groundtruth1Dz(timey);
[PP3, VV3, AA3] = groundtruth1Dy(timez);

figure
subplot(311), plot(accxt, accx);
subplot(312), plot(accyt, accy);
subplot(313), plot(acczt, accz);

figure
subplot(311), plot(timex, AA1);
subplot(312), plot(timey, AA2);
subplot(313), plot(timez, AA3);


% 1D - 3D coordinates
% td3
%td3 = 0:T:105;
A3x =  zeros(1, length(accxt));
A3y =  zeros(1, length(accyt));
A3z =  zeros(1, length(acczt));

%xS = 3.6; yS = 46.1784; zS = 95.5027;

% x
%A3x(1,round(xS/T)+1:length(AA1) + round(xS/T)) = AA1;
A3x(1,xS+1:xS+length(AA1)) = AA1;

%y
%A3y(2,round(yS/T)+1:length(AA2) + round(yS/T)) = AA2;
A3y(1,zS - z1s + 1:zS - z1s + length(AA2)) = AA2;

%z
%A3z(3,round(zS/T)+1:length(AA3) + round(zS/T)) = AA3;
A3z(1,yS - y1s + 1:yS - y1s + length(AA3)) = AA3;

% A3(3,length(AA3)*3+1:length(AA3)*4) = flip(AA3);
A3x = A3x + 0.14273*ones(1, length(accxt));
A3y = A3y ;
A3z = A3z + 0.650848*ones(1, length(acczt)) - 0.0005*acczt';% + 0.14273*ones(1, length(timez));

A3x_sim = awgn(A3x,30,'measured');
A3y_sim = awgn(A3y,13,'measured');
A3z_sim = awgn(A3z,30,'measured');

figure
subplot(311), plot(accxt, A3x_sim);
subplot(312), plot(accyt, A3y_sim);
subplot(313), plot(acczt, A3z_sim);

[mean(A3x_sim - A3x), rms(A3x_sim - A3x), sqrt(rms(A3x_sim - A3x))]
[mean(A3z_sim - A3z), rms(A3z_sim - A3z), sqrt(rms(A3z_sim - A3z))]
[mean(A3y_sim - A3y), rms(A3y_sim - A3y), sqrt(rms(A3y_sim - A3y))]

[mean(accx' - A3x), rms(accx' - A3x), sqrt(rms(accx' - A3x))]
[mean(accz' + A3z), rms(accz' + A3z), sqrt(rms(accz' + A3z))]
[mean(accy' - A3y), rms(accy' - A3y), sqrt(rms(accy' - A3y))]

% figure
% subplot(611),plot(time, A3_sim(1,:),'LineWidth',2), xlim([0,100]);%hold on, plot(acc_time, accx,'LineWidth',2), xlabel('time [s]'), ylabel('x axis ($m/s^2$)','interpreter','latex'),title('Simulated Accelerations Along x Axe Based on Error Model')
...
% subplot(613),plot(time, A3_sim(2,:),'LineWidth',2), %hold on, plot(acc_time, accz,'LineWidth',2), xlabel('time [s]'), ylabel('y axis ($m/s^2$)','interpreter','latex'),title('Simulated Accelerations Along y Axe Based on Error Model')
...
% subplot(615),plot(time, A3_sim(3,:),'LineWidth',2), %hold on, plot(acc_time, accy,'LineWidth',2), xlabel('time [s]'), ylabel('z axis ($m/s^2$)','interpreter','latex'),title('Simulated Accelerations Along z Axe Based on Error Model')
...
figure
subplot(311),plot(accxt, accx,'LineWidth',3),hold on, plot(accxt, A3x_sim,'LineWidth',1),  hold on, plot(accxt, A3x,'LineWidth',2),  xlim([0,10]); xlabel('time [s]'), ylabel('x axis ($m/s^2$)','interpreter','latex'),legend('Measurement', 'Simulated', 'Ground Truth with Bias');title('Simulated Accelerations Along x Axe Based on Error Model')
subplot(312),plot(acczt, accz,'LineWidth',3),hold on, plot(acczt, -A3z_sim,'LineWidth',1), hold on, plot(acczt, -A3z,'LineWidth',2), xlim([45,55]); xlabel('time [s]'), ylabel('y axis ($m/s^2$)','interpreter','latex'),legend('Measurement', 'Simulated', 'Ground Truth with Bias');title('Simulated Accelerations Along y Axe Based on Error Model')
subplot(313),plot(accyt(10:length(accyt)), accy(10:length(accy)),'LineWidth',3),hold on,plot(accyt, A3y_sim,'LineWidth',1),hold on,plot(accyt, A3y,'LineWidth',2),xlim([90,100]); xlabel('time [s]'), ylabel('z axis ($m/s^2$)','interpreter','latex'),legend('Measurement', 'Simulated', 'Ground Truth with Bias');title('Simulated Accelerations Along z Axe Based on Error Model')
