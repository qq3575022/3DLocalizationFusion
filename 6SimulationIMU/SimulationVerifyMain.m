...
% x
coord3(1,1:xS) = 1.03;
coord3(1,xS+1:length(AA1)+xS) = PP1 + 1.03;
coord3(1,length(AA1)+xS+1:end) = PP1(end) + 1.03;

%y
coord3(2,1:yS) = 1.03;
coord3(2,yS+1 : length(AA2)+yS) = PP2 + 1.31;
coord3(2,length(AA2)+yS+1:end)  = PP2(end) + 1.31;

%z
coord3(3,1:zS) = 1.03;
coord3(3,zS+1 : length(AA3)+zS) = PP3 + 1.03;
coord3(3,length(AA3)+zS+1:end)  = PP3(end) + 1.03;
...
[H1(k+1),phi_mu1(k+1),r_sim1(k+1),rdot_sim1(k+1),diff1(k+1)] = noisysim(x1,f1,Gt,M,X,PT,GT,GR,R,sigma,1,k,z,z_prev,acc_time(k+1)-acc_time(k));
[H2(k+1),phi_mu2(k+1),r_sim2(k+1),rdot_sim2(k+1),diff2(k+1)] = noisysim(x2,f2,Gt,M,X,PT,GT,GR,R,sigma,2,k,z,z_prev,acc_time(k+1)-acc_time(k));
[H3(k+1),phi_mu3(k+1),r_sim3(k+1),rdot_sim3(k+1),diff3(k+1)] = noisysim(x3,f3,Gt,M,X,PT,GT,GR,R,sigma,3,k,z,z_prev,acc_time(k+1)-acc_time(k));   
[H4(k+1),phi_mu4(k+1),r_sim4(k+1),rdot_sim4(k+1),diff4(k+1)] = noisysim(x4,f4,Gt,M,X,PT,GT,GR,R,sigma,4,k,z,z_prev,acc_time(k+1)-acc_time(k));  
end
