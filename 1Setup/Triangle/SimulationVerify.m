...
if sim == 1
    ...
    legend({'Measurement','Simulation','Ground truth'},'Location', 'SouthEast')
    xlabel('t [s]')

    subplot(2,1,2),
    plot(td,vv,'-',td,rdot_sim,'LineWidth',2); hold on; plot(td,VV,'LineWidth',1);ylabel('Velocity [m/s]');title('1D Velocity');
    legend({'Measurement','Simulation','Ground truth'},'Location', 'NorthEast')
    xlabel('t [s]')
...
%     subplot(2,1,2),plot(td,phi_conca); title('phi concatenated');xlabel('t [s]')
    ...
    subplot(5,1,3),plot(td,W,'LineWidth',2),  title('Orientation $\psi$','interpreter','latex','fontweight','bold'),     ylabel('$\psi$ [rad]','interpreter','latex'),     xlabel('t [s]'), grid%,xlim([0,td(end)])
    ...
    figure
    subplot(3,1,1),plot(td,r1f,'-',td,r_sim1,'LineWidth',2);hold on;plot(td,rr(1,:),'LineWidth',1);legend({'Measurement','Simulated','Ground truth'},'Location', 'SouthEast');title('2D Radial Distance $r_1$','interpreter','latex');ylabel('radial distance [m]');xlabel('t [s]')
    subplot(3,1,2),plot(td,r2f,'-',td,r_sim2,'LineWidth',2);hold on;plot(td,rr(3,:),'LineWidth',1);legend('Measurement','Simulated','Ground truth');title('2D Radial Distance $r_2$','interpreter','latex');ylabel('radial distance [m]');xlabel('t [s]')
    subplot(3,1,3),plot(td,r3f,'-',td,r_sim3,'LineWidth',2);hold on;plot(td,rr(5,:),'LineWidth',1);legend({'Measurement','Simulated','Ground truth'},'Location', 'SouthEast');title('2D Radial Distance $r_3$','interpreter','latex');ylabel('radial distance [m]');xlabel('t [s]')
    ...
    subplot(3,1,2),plot(td,r2dot_e,'-',td,rdot_sim2,'LineWidth',2);hold on;plot(td,rr(4,:),'LineWidth',1);legend({'Measurement','Simulated','Ground truth'},'Location', 'SouthEast');title('2D Radial Velocity $\dot r_2$','interpreter','latex');ylabel('radial velocity [m/s]');xlabel('t [s]')
    subplot(3,1,3),plot(td,r3dot_e,'-',td,rdot_sim3,'LineWidth',2);hold on;plot(td,rr(6,:),'LineWidth',1);legend({'Measurement','Simulated','Ground truth'},'Location', 'NorthEast');title('2D Radial Velocity $\dot r_3$','interpreter','latex');ylabel('radial velocity [m/s]');xlabel('t [s]')

    figure
    subplot(4,1,1),plot(td,0.8*(ax1+0.6),'LineWidth',2);hold on; plot(td,ax_noisy,'LineWidth',2);hold on; plot(td,rr(7,:),'LineWidth',2);legend({'Measurement','Simulated','Ground truth'},'Location', 'SouthEast');title('Acceleration $a_x$','interpreter','latex');ylabel('acceleration [m/$s^2$]','interpreter','latex');xlabel('t [s]')
    subplot(4,1,2),plot(td,0.8*(ay1+0.15),'LineWidth',2);hold on; plot(td,ay_noisy,'LineWidth',2);hold on; plot(td,rr(8,:),'LineWidth',2);legend('Measurement','Simulated','Ground truth');title('Acceleration $a_y$','interpreter','latex');ylabel('acceleration [m/$s^2$]','interpreter','latex');xlabel('t [s]')
    subplot(4,1,3),plot(td, 0.85*wf, td, W_VN, td, W_V, 'LineWidth',2),legend('Measurement','Simulated','Ground truth'),title('Angular Velocity $\theta_z$','interpreter','latex','fontweight','bold'),ylabel('gyroscope output $\theta_z$ [rad/$s$]','interpreter','latex'),xlabel('t [s]')
    subplot(4,1,4),plot(td, psif1, td, WN, td, W, 'LineWidth',2),legend({'Measurement','Simulated','Ground truth'},'Location', 'SouthEast'),title('Orientation $\omega_z$','interpreter','latex','fontweight','bold'),ylabel('magnetometer output $\omega_z$ [rad]','interpreter','latex'),xlabel('t [s]')
