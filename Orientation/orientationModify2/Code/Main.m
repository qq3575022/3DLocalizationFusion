...
%[phi,phi_gt,AXY,XX,Tmag,tdmag,Tacc,tdacc,T3,t_x] = groundtruth3D();
% W = zeros(3, length(phi));
% W(1,:) = phi(1,:);W(2,:) = phi(3,:);W(3,:) = phi(5,:);
% 
% W_V = zeros(3, length(phi));
% W_V(1,:) = p hi(2,:);W_V(2,:) = phi(4,:);W_V(3,:) = phi(6,:);

% [t_v, T_v, W_V, t_m, T_m, W, t_a, T_a, a, t_x, XX] = getPath();
% [t_v, W_V, t_m, W, t_a, AXY, t_x, XX] = sensor(t_v, T_v, W_V, t_m, W, t_a, T_a, a, t_x, XX);

[gyro,AXY,Tgyro,tdgyro,Tacc,tdacc] = meas3D();
