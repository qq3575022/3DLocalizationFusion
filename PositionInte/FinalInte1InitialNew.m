clc, clear, close all
%data=readtable('../Data/0304IMU/whole/2second.csv','Delimiter', ',');  g=9.7953;
...
% ******************************{ //1 Comment out // ******************************
biasx = 0.014;     biasy = 0.03;    biasz = 0.065;
...
driftxSep = 0.097; driftySep = 0.03; driftzSep = 0.01; 
...
% 
% 
% posYFinal  = doubleInte(timeDiff(yS:BEY), acczCali(yS:BEY)-0.04, 2);

% figure
% plot(posYFinal)
% 
% figure
% subplot(611), plot(acc_time(xS:xE), posX)
% subplot(612), plot(acc_time(BS:BE), posX2)
% 
% subplot(613), plot(acc_time(yS:yE), posY)
% subplot(614), plot(acc_time(BS:BE),posY2)
% 
% 
% subplot(615), plot(acc_time(zS:zE), posZ)
% subplot(616), plot(acc_time(BS:BE), posZ2)


figure;
plot(acc_time(2:length(acc_time)), accx(2:length(accx)), 'LineWidth', 2),xlim([0,100])
hold on
plot(acc_time(2:length(acc_time)), accy(2:length(accx)), 'LineWidth', 2),xlim([0,100])
hold on
plot(acc_time(2:length(acc_time)), accz(2:length(accx)), 'LineWidth', 2),xlim([0,100])
xlabel('time [s]', 'FontSize', 18)
ylabel('acceleration [m/s^2]', 'FontSize', 18)
legend('x-aixs','y-aixs','z-aixs', 'Location', 'SouthEast', 'FontSize', 18)
title('accelerometer data', 'FontSize', 18)
