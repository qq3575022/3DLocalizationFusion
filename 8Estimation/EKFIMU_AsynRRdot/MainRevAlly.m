...
% start time
yS = find(abs(acc_time-46.13)<0.002); yS = yS(1);
yE = find(abs(acc_time-48.5)<0.002); yE = yE(1);

yyS = find(abs(gyro_time-46.13)<0.002);  yyS = yyS(1);
yyE = find(abs(gyro_time-48.5)<0.002); yyE = yyE(1);

ySS = find(abs(mag_time-46.13)<0.009);   ySS = ySS(1);
...
accx = acc_data_x(yS:yE) - mean(acc_data_x(yS:yE));
accy = acc_data_y(yS:yE) - mean(acc_data_y(yS:yE));
accz = acc_data_z(yS:yE) - mean(acc_data_z(yS:yE));
accT = acc_time(yS:yE);

gyrox = gyro_data_x(yyS:yyE) - mean(gyro_data_x(yyS:yyE));
gyroy = gyro_data_y(yyS:yyE) - mean(gyro_data_y(yyS:yyE));
gyroz = gyro_data_z(yyS:yyE) - mean(gyro_data_z(yyS:yyE));
gyroT = gyro_time(yyS:yyE);


magx = mag_data_x(ySS:yEE) - mean(mag_data_x(ySS:yEE));
magy = mag_data_y(ySS:yEE) - mean(mag_data_y(ySS:yEE));
magz = mag_data_z(ySS:yEE) - mean(mag_data_z(ySS:yEE));
...
%%
% figure
% subplot(311), plot(time, x(1,:), 'LineWidth', 2); hold on; plot(accT(2:end), posX, 'LineWidth', 2); legend('estimated','integration'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('position [m]'); grid on; grid minor; ylim([-0.3, 0.2]); xlim([46, 48.5]);%hold on; plot(time, XX(1,:))
% subplot(312), plot(time, x(7,:), 'LineWidth', 2); hold on; plot(accT(2:end), posY, 'LineWidth', 2); hold on; plot(time, -PP1, 'LineWidth', 2); legend('estimated','integration','ground truth'); title('Position Along y Axis [m]'); xlabel('t [s]'); ylabel('position [m]'); grid on; grid minor; xlim([46, 48.5]);%hold on; plot(time, XX(4,:))
% subplot(313), plot(time, x(4,:), 'LineWidth', 2); hold on; plot(accT(2:end), posZ, 'LineWidth', 2); legend('estimated','integration'); title('Position Along z Axis [m]'); xlabel('t [s]'); ylabel('position [m]'); grid on; grid minor; xlim([46, 48.5]);%hold on; plot(time, XX(7,:))

figure
subplot(311), plot(time, x(7,:), 'LineWidth', 3); hold on; plot(time, -PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','NorthEast'); title('Position Along y Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([46.13, 48.5]);
subplot(312), plot(time, x(8,:), 'LineWidth', 2); hold on; plot(time, -VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth','Location','SouthEast'); title('Velocity Along y Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([46.13, 48.5]);
subplot(313), plot(time, x(9,:), 'LineWidth', 2); hold on; plot(time, -AA1, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth','Location','SouthEast'); title('Acceleration Along y Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([46.13, 48.5]);
