...
% covariance for v
RR = [0.03*var(magx), 0.03*var(gyrox), 0.03*var(magy), 0.03*var(gyroy), 0.03*var(magz), 0.03*var(gyroz), 0.03*var(accx), 0.03*var(accy), 0.03*var(accz)]; 
...
[pos, vel] = doubleInte(diff(time), x(3,2:end));

figure
subplot(311), plot(time(2:end), pos, 'LineWidth', 3); hold on; plot(time, PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([3.5, 7.5])
