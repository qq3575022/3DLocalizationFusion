...
[pos, vel] = doubleInte(diff(time), x(9,2:end));

figure
subplot(311), plot(time(2:end), pos, 'LineWidth', 3); hold on; plot(time, -PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','NorthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([46.13, 48.5]);
subplot(312), plot(time(2:end), vel, 'LineWidth', 2); hold on; plot(time, -VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth','Location','SouthEast'); title('Velocity Along x Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([46.13, 48.5]);
